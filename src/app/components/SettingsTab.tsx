...
export function SettingsTab({ isMobile = false }: { isMobile?: boolean }) {
  ...
  // Email Config State
  const [emailEnabled, setEmailEnabled] = useState(() => localStorage.getItem('smart_care_email_enabled') === 'true');
  const [emailReceiver, setEmailReceiver] = useState(() => localStorage.getItem('smart_care_email_receiver') || '');
  ...
  const syncEmailConfig = async () => {
    if (!backendUrl) return;
    const cleanUrl = backendUrl.endsWith('/') ? backendUrl.slice(0, -1) : backendUrl;
    try {
      await fetch(`${cleanUrl}/set_email_config`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      console.error("Email Config Sync Error:", err);
    }
  };
  ...
  const handleTestEmail = async () => {
    ...
    const cleanUrl = backendUrl.endsWith('/') ? backendUrl.slice(0, -1) : backendUrl;
    try {
      const res = await fetch(`${cleanUrl}/test_email`);
      const data = await res.json();
      if (data.success) {
        alert(`✅ 測試件已發送！請檢查您的收件匣。`);
      } else {
  ...
  return (
    <div className="flex flex-col gap-6 px-4 pb-24 relative">
      ...
      {/* Email Config */}
      <SettingGroup title="電子郵件通知" icon={Mail}>
        <SettingRow label="啟用郵件通知">
        ...
        {emailEnabled && (
          ...
          >
            <SettingRow label="收件人信箱" description="警報發生時接收通知的電子郵件。">
              <input
                className="settings-input"
                value={emailReceiver}
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-[11px] font-bold shadow-md active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                <Mail size={14} /> 發送測試郵件
              </button>
